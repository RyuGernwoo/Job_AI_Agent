...
def build_generation_prompt(
    *,
    ...
    return (
        ...
        "Content depth: produce substantive, classroom-ready Korean that a trainer could teach from directly, not "
        "one-line placeholders. Each lecture_flow 'content' must be 3-6 sentences that (1) explain the core concept "
        "grounded in the evidence, (2) walk through a concrete worked example or procedure taken from the evidence, "
        "and (3) state what the learner does or verifies. Each practice step must be an actionable instruction paired "
        "with its expected result. The performance_task 'description' must state the task, the required deliverable, "
        "and what is evaluated (at least 3 sentences). Each multiple_choice 'explanation' must justify why the correct "
        "option is right and briefly why a plausible distractor is wrong (at least 2 sentences). Favor specific, "
        "concrete wording over generic filler, but never introduce facts, numbers, tools, or sources absent from the "
        "evidence. "

...
| Performance | RAG 근거 검색, 평가 문항 생성, 강사 검수 | 품질이 높을수록 만족도가 상승 |
...
| 강사 검수 | 생성 결과, 근거 문단, 수정 에디터, 승인/재생성 버튼 |
...
| 강사 검수·수정 | 강사로서 AI 초안을 수정하고 승인하고 싶다. | Given 생성 결과가 draft 상태일 때, When 수정하고 승인하면, Then 상태가 approved로 저장된다. |
| 문서 다운로드 | 강사로서 승인된 결과를 DOCX로 내려받고 싶다. | Given 패키지가 approved 상태일 때, When DOCX 다운로드를 누르면, Then 교안·실습·평가가 포함된 파일이 생성된다. |
...
| KPI | 측정 방식 | MVP 목표 |
|---|---|---|
| E2E 성공률 | 업로드→생성→검수→DOCX 다운로드 완료 여부 | 데모 시나리오 1개 성공 |
| 근거 연결률 | 생성 항목 중 citation ID가 붙은 항목 비율 | 핵심 항목 90% 이상 |
| 문서 생성 성공률 | approved 패키지에서 DOCX 파일 생성 여부 | 100% |
| 수정 가능성 | 생성 결과를 수정·저장할 수 있는지 | 강사 검수 후 저장 가능 |
...
개발 로드맵은 기능을 넓히는 것이 아니라 **1개 차시 end-to-end 성공**을 중심으로 설계한다. MVP가 보여줘야 할 핵심은 “AI가 그럴듯한 텍스트를 만든다”가 아니라, “근거 기반 생성 → 사람 검수 → 운영 문서화” 흐름이다.

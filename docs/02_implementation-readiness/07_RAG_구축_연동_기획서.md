...
| 외부 Supabase migration 적용 | 운영 작업 필요 | SQL Editor에서 `002_rag_persistence.sql` 실행 |
| 1536차원 semantic embedding 재색인 | 운영 전환 시 수행 | migration 후 환경변수 전환 및 ingestion 재실행 |
...
| RAG-05 | 현재 임베딩은 64차원 토큰 해시 방식 | 한국어 의미 유사도와 동의어 검색 성능이 제한됨 | 운영에서는 단일 외부 임베딩 모델로 재색인 |
...
단계 1~3은 데이터 모델 변경이므로 개발 Supabase 프로젝트에서 먼저 수행한다. 운영 테이블을 직접 삭제하거나 기존 43개 chunk를 덮어쓰지 않는다. 재색인은 새 embedding version 확인 후에만 실행한다.
...
1. migration을 개발 Supabase에 적용한다.
2. 43개 baseline chunk를 새 embedding version으로 재색인하고 retrieval report를 저장한다.
3. `/rag/retrieve`와 `/rag/generate` 통합 테스트를 실제 Supabase·LLM 환경에서 실행한다.
4. Langfuse에서 retrieval span, generation span, `retrieval_run_id`를 확인한다.

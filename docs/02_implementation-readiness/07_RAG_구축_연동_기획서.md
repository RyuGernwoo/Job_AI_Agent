...
6. baseline에 관련 NCS 자료가 없어도 현재 프로젝트의 실제 업로드 chunk를 근거로 생성할 수 있다.
...
| RAG-07 | baseline에 없는 NCS 분야는 검색 0건 또는 무관한 공통 자료가 선택될 수 있음 | 신규 분야 생성 실패 또는 근거 오염 | 업로드 chunk fallback, 프로젝트 우선 선택, baseline 최소 관련도 적용 |
...
프로젝트 자료로 top-k를 먼저 채우고 부족한 경우에만 baseline 자료를 보충한다. 동일한 내용을 두 범위에서 찾은 경우 프로젝트 자료를 우선한다. 검색 결과에는 `scope` (`project` 또는 `baseline`)와 `strategy` (`hybrid` 또는 `project_material_fallback`)를 추가해 UI와 로그에서 구별한다.

### 4.1.1 baseline 미지원 분야 fallback

1. 사용자 질의에 과정명, 차시명, 학습목표, NCS 코드·명칭·요소를 결합한다.
2. 현재 `project_id`의 업로드 자료를 먼저 의미·어휘 검색한다.
3. 검색 결과가 0건이거나 임베딩 검색이 실패하면 현재 프로젝트에 실제 저장된 chunk를 문서별로 고르게 선택한다.
4. fallback chunk에는 `evidence_origin=user_upload`, `evidence_authority=user_provided`, `retrieval_strategy=project_material_fallback`을 남긴다.
5. fallback 전략에서는 baseline을 생략한다. 일반 hybrid 검색에서는 프로젝트 결과로 채우지 못한 슬롯만 baseline으로 보충하고 combined score `0.20` 미만 결과는 제외한다.
6. 생성 프롬프트는 사용자 입력 NCS 정보를 기획 제약으로 취급하고, 공식 NCS 출처가 없는 성능 기준을 임의로 만들지 않도록 제한한다.

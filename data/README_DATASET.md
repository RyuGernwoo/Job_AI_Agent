...
NCS 운영 데이터는 다음 세 계층을 구분한다.

| 계층 | 저장 위치 | 의미 |
| --- | --- | --- |
| 공식 능력단위 catalog | `lessonpack_ncs_catalog` | 코드·명칭·분류 검색 기준, 수행준거 0개 가능 |
| 상세 criteria·공통 RAG | `lessonpack_ncs_criteria`, `lessonpack_chunks` | 상세 수행준거와 학습모듈 근거 |
| 프로젝트 업로드 자료 | 프로젝트별 `lessonpack_chunks` | 공통 상세 RAG가 없는 분야의 우선 근거 |

공식 API가 연결되어도 `catalog`와 `detail` 동기화는 별도다. 화면의 `수행준거 0개 · RAG 미적재`는 catalog에는 존재하지만 상세 criteria와 임베딩이 아직 준비되지 않았다는 뜻이다.
...
- 능력단위 코드와 API·DB 검색 키에는 `LM`을 붙이지 않은 전체 버전 코드(예: `2001020231_23v5`)를 사용한다.
- `LM`은 학습모듈 문서 식별자와 파일명(예: `LM2001020231_23v5_programming_language_use.md`)에만 사용한다.
- 이전 입력과의 호환을 위해 API는 `LM2001020231_23v5`도 받아들이지만, 저장·응답 시 `2001020231_23v5`로 정규화한다.
...
분야별 chunk는 사업관리 4,696개, 경영·회계·사무 6,503개, 금융·보험 7,904개다. 이 수치는 2026-07-23에 검증한 원본 확장 적재 이력이며 공식 API의 현재 전체 백필 수량을 뜻하지 않는다. 기존 43개 MVP chunk와 같은 `mvp-dataset` 범위로 Supabase에 적재되어 서비스 검색 시 함께 사용된다. 로컬 재구축이 필요하면 공식 NCS API 동기화를 우선 사용하고, 원문 PDF/XLS 재처리는 별도 원본을 다시 확보한 경우에만 수행한다.
